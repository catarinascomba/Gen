...
/** Additional parameters for signal types **/
int dc;
...
int main(int argc, char *argv[])
{
    ...
    /* duty cycle parsing */
    double dutycycle = strtod(argv[5], NULL);
    ...
    synthesize_signal(ampl, freq, type, endfreq, data, &params, dutycycle);
...
void synthesize_signal(double ampl, double freq, signal_e type, double endfreq,
    ...
    for(i = 0; i < n; i++) {
        ...
        if (type == eSignalPWM)
        {         
            // calculate duty cycle -> 25 = 0.25
            double ddc = (double) dc / 100;     
            int nbuff = 16*1024;          
